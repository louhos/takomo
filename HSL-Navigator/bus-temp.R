...
p1 <- ggplot(temp, aes(x=Rank, y=timediff, group=Trip)) + geom_path(alpha=0.5) + geom_hline(y=c(-1, 3), linetype="dashed", colour="red", size=1) + xlab("BusStop") + ylab("Observed - Schedule (min)") + ggtitle(paste("Route: ", temp2$Route[1], " (route id: ", route.id,")",sep="")) + facet_wrap(~Direction, nrow=2) + ylim(-10, 20)
...
dat2 <- merge(dat.backup, stops, by.x="Location", by.y="stop_id", all.x=TRUE)
