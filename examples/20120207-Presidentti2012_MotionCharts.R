...
# Remember to install required packages (e.g. 'install.packages("googleVis")')
library(googleVis)
library(XML)
...
mchart.pks <- googleVis::gvisMotionChart(pks.votes, idvar="Alue", timevar="Aika", options=list(height=600, width=700))
...
mchart.mun <- googleVis::gvisMotionChart(mun.votes, idvar="Alue", timevar="Aika", options=list(height=600, width=700))
# Plot immediately (opens in browser)
plot(mchart.mun)
# Save as html (needs javascript to open!)
